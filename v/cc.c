	...
	#define V_COMMIT_HASH "993802f"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "993802f"
#endif
...
typedef struct main__Employee main__Employee;
...
typedef struct _option_main__Employee _option_main__Employee;
...
struct _option_main__Employee {
	byte state;
	IError err;
	byte data[sizeof(main__Employee) > 1 ? sizeof(main__Employee) : 1];
};
...
_option_main__Employee json__decode_main__Employee(cJSON* root);
cJSON* json__encode_main__Employee(main__Employee val);
...
VV_LOCAL_SYMBOL int compare_13010696017967530487_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
_option_voidptr json__decode(voidptr typ, string s);
...
VV_LOCAL_SYMBOL void main__main_json(void);
...
_option_main__Employee json__decode_main__Employee(cJSON* root) {
	main__Employee res = ((main__Employee){.name = (string){.str=(byteptr)"", .is_lit=1},.age = 0,.salary = 0,});
	if (!root) {
		...
		if (error_ptr != NULL)	{
			...
			return (_option_main__Employee){.state = 2,.err = _v_error(tos2(buf)),.data = {0}};
		}
	}

	cJSON *jsonroot__t1 = js_get(root, "name");
	if (jsonroot__t1) {
		res.name = json__decode_string(jsonroot__t1);
	}
	cJSON *jsonroot__t2 = js_get(root, "age");
	if (jsonroot__t2) {
		res.age = json__decode_int(jsonroot__t2);
	}
	cJSON *jsonroot__t3 = js_get(root, "salary");
	if (jsonroot__t3) {
		res.salary = json__decode_f32(jsonroot__t3);
	}
	_option_main__Employee ret;
	opt_ok2(&res, (_option*)&ret, sizeof(res));
	return ret;
}
...
cJSON* json__encode_main__Employee(main__Employee val) {
	...
	cJSON_AddItemToObject(o, "age", json__encode_int(val.age));

	cJSON_AddItemToObject(o, "salary", json__encode_f32(val.salary));
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	...
	if (sidx == _IError_time__TimeParseError_index) return 118;
	return 29;
}
...
_option_int strconv__atoi(string s) {
	if ((s).len == 0) {
		return (_option_int){ .state=2, .err=_v_error(_SLIT("strconv.atoi: parsing \"\": invalid syntax")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
...
string string_replace_each(string s, Array_string vals) {
	...
	qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_13010696017967530487_RepIndex_by_idx);
	u8* b = malloc_noscan(new_len + 1);
	int idx_pos = 0;
...
_option_voidptr json__decode(voidptr typ, string s) {
	_option_voidptr _t1;
	opt_ok2(&(voidptr[]) { 0 }, (_option*)(&_t1), sizeof(voidptr));
	return _t1;
}
...
_option_time__Time time__parse_rfc3339(string s) {
	...
	if (!string_contains((*(string*)array_get(parts, 0)), _SLIT("-")) && string_contains((*(string*)array_get(parts, 0)), _SLIT(":"))) {
		...
		_option_multi_return_int_int_int_int_i64_bool _t8 = time__parse_iso8601_time((*(string*)array_get(parts, 0)));
		if (_t8.state != 0) {
...
_option_time__Time time__parse_iso8601(string s) {
	...
	_option_multi_return_int_int_int _t4 = time__parse_iso8601_date((*(string*)array_get(parts, 0)));
	if (_t4.state != 0) {
	...
	bool is_local_time = true;
	if (parts.len == 2) {
		_option_multi_return_int_int_int_int_i64_bool _t6 = time__parse_iso8601_time((*(string*)array_get(parts, 1)));
		if (_t6.state != 0) {
			_option_time__Time _t7;
			memcpy(&_t7, &_t6, sizeof(_option));
			return _t7;
...
VV_LOCAL_SYMBOL void main__main_json(void) {
	string s = _SLIT("{\"name\":\"Peter\",\"age\":28,\"salary\":95000.5,\"title\":2}");
	// json.decode
	cJSON* _t2 = json__json_parse(s);
	_option_main__Employee _t3 = json__decode_main__Employee(_t2);
	cJSON_Delete(_t2); // del
	_option_main__Employee _t1 = (_t3);
	if (_t1.state != 0) {
		IError err = _t1.err;
		_v_panic(_SLIT("err"));
		VUNREACHABLE();
	;
 	...
 	main__Employee y =  (*(main__Employee*)_t1.data);
	{main__Employee _ = y;}
	;
}
...
VV_LOCAL_SYMBOL void main__main(void) {
	...
	time__Duration d = time__since(t);
	i64 mill_sec = time__Duration_milliseconds(d);
	println( str_intp(2, _MOV((StrIntpData[]){{_SLIT("total "), /*100 &i64*/0xfe09, {.d_i64 = mill_sec}}, {_SLIT(" in ms"), 0, { .d_c = 0 }}})));
}
